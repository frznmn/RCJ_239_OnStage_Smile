...
void display_panOrBounceBitmap (uint8_t bitmapSize) {
	...
	// pan 24x24 pixmap
	if (bitmapSize == 24) matrix->drawRGBBitmap(x, y, (const uint16_t *) sprite, bitmapSize, bitmapSize);
#ifdef BM32
...
//*/
int bmx=0,bmy=0;
    matrix->fillRect(bmx,bmy, bmx+16,bmy+8, LED_BLACK);
    //matrix->drawBitmap();
    matrix->drawRGBBitmap(0, 0, (const uint16_t *) sprite, 16, 8);
